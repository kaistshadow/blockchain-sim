    ...
    public:
        ...
        void ChurnOut() {
            ...
            if(!_listen_socket) {
                ...
                std::cout<<"Churn out called for not listening socket";
                exit(1);
            }
            if(_listen_socket == -1) {
                perror("Churn out called for already churned out socket");
                std::cout<<"Churn out called for already churned out socket";
                exit(1);
            }
        ...
        void ChurnToggle() {
            ...
            std::strftime(&s[0], s.size(), "%Y-%m-%d %H:%M:%S", std::localtime(&now));
            if(!isChurnedIn()) {
                std::cout << "churn toggle is called at " << Node<NodePrimitives>::GetIP() << " - churning in at " << s << " ... ";
                ChurnIn();
                std::cout << "done\n";
            }
            else {
                std::cout << "churn toggle is called at " << Node<NodePrimitives>::GetIP() << " - churning out at " << s << " ... ";
                ChurnOut();
                std::cout << "done\n";
            }
        }

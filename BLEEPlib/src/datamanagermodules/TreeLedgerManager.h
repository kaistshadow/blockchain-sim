#ifndef TREE_LEDGER_MANAGER_H
#define TREE_LEDGER_MANAGER_H
...
namespace libBLEEP {
    ...
    template <typename T> 
        class TreeLedgerManager {
        static_assert(std::is_base_of<Block, T>::value, "T must inherit from Block");
    protected:
        std::unordered_map<std::string, std::shared_ptr< TreeBlockIndex<T> > > _ledgermap;
        int _treeheight = 0;
    ...
    public:
        ...
        Inventory GetProcessingInv() { return _inv; }
        void SetProcessingInv(Inventory inv) { _inv = inv; }


        std::string GetChildBlockHash(std::string hash) {
            auto it = find_if(_ledgermap.begin(), _ledgermap.end(), 
                              [hash](const std::pair<std::string, std::shared_ptr< TreeBlockIndex<T> > > & t) -> bool {
                                  return (t.second->GetPrevBlockHash() == hash);
                              });

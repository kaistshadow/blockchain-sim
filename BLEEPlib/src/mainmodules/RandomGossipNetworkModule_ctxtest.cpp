#include "RandomGossipNetworkModule_ctxtest.h"
#include "../utility/Random.h"

using namespace libBLEEP;

RandomGossipNetworkModule_ctxtest::RandomGossipNetworkModule_ctxtest(std::string myPeerId, MainEventManager* mainEventManager, int fanOutNum)
...
bool RandomGossipNetworkModule_ctxtest::InsertMessageSet(std::string messageId) {
    return messageSet.insert(messageId).second;
}

bool RandomGossipNetworkModule_ctxtest::ExistMessage(std::string messageId) {
    auto itr = messageSet.find(messageId);
    return (itr != messageSet.end());
}

bool RandomGossipNetworkModule_ctxtest::AsyncConnectPeer(PeerId id, double time) {
...
std::set<Distance, DistanceCmp> genNeighborPeerSet_ctxtest(PeerId myId, std::vector<PeerId> &neighborPeerIds){
...
bool RandomGossipNetworkModule_ctxtest::AsyncConnectPeers(std::vector<PeerId> &peerList, int peerNum, double time){
    char buf[256];
    ...
    auto neighborPeerIdSet = genNeighborPeerSet_ctxtest(myId, peerList);
    int i = 0;
    for(const Distance& dest : neighborPeerIdSet){
        if (i >= peerNum) break;
        if (RandomGossipNetworkModule_ctxtest::AsyncConnectPeer(dest.GetPeerId(), time) == true)
            i++;
    }
    return true;
}

bool RandomGossipNetworkModule_ctxtest::UnicastMessage(PeerId dest, std::shared_ptr<Message> message) {
...
bool RandomGossipNetworkModule_ctxtest::DisconnectPeer(PeerId id) {
...
bool checkSourcePeer_ctxtest(std::shared_ptr<Message> msg, PeerId peerId){
    return (msg->GetSource().GetId() != peerId.GetId());
}

bool RandomGossipNetworkModule_ctxtest::SendMulticastMsg(PeerId dest, std::shared_ptr<Message> message) {
...
bool RandomGossipNetworkModule_ctxtest::MulticastMessage(std::shared_ptr<Message> message){
    char buf[256];
...
bool RandomGossipNetworkModule_ctxtest::ForwardMessage(std::shared_ptr<Message> message){
    char buf[256];
...
std::vector<PeerId> RandomGossipNetworkModule_ctxtest::GetNeighborPeerIds(PeerConnectMode mode){
    return peerManager.GetNeighborPeerIds(mode);
}

PeerId RandomGossipNetworkModule_ctxtest::GetMyPeerId(){

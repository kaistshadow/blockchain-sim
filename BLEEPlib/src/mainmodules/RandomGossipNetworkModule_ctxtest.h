...
namespace libBLEEP {
    ...
    class RandomGossipNetworkModule_ctxtest {
    private:
        int fanOut;
        ...
        protected:
            RandomGossipNetworkModule_ctxtest* _networkModule;
            MainEventManager* _mainEventModule;
        public:
            WatcherWrapper(RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
        ...
        public:
            WatcherManager(RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
                : _networkModule(netModule), _mainEventModule(eventModule) {};
        ...
        public:
            AsyncConnectTimer(PeerId id, double time, RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
        ...
        public:
            ListenSocketWatcher(int fd, RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
                : WatcherWrapper(netModule, eventModule) {
        ...
        private:
            ...
            void _dataSocketIOCallback(ev::io &w, int revents) {
                ...
                if (revents & EV_READ) {
                    ...
                    bool isIdleBefore = (dataSocket->GetRecvBuff().recv_status == RECV_IDLE);
                    std::pair<bool, std::shared_ptr<Message>> recvResult = dataSocket->DoRecv();
                    bool isMsgAfter = (dataSocket->GetRecvBuff().recv_status == RECV_MSG);
                    if (!recvResult.first) {
        ...
        public :
            DataSocketWatcher(int fd, RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
                : WatcherWrapper(netModule, eventModule) {
        ...
        public:
            ConnectSocketWatcher(int fd, RandomGossipNetworkModule_ctxtest* netModule, MainEventManager* eventModule)
                : WatcherWrapper(netModule, eventModule) {
    ...
    public:
        RandomGossipNetworkModule_ctxtest(std::string myPeerId, MainEventManager* worker, int fanOutNum);

...
void BLNodePrimitives::OpAfterConnect(int conn_fd) {
    ...
    std::shared_ptr<libBLEEP_BL::Message> peerIDmsg = std::make_shared<libBLEEP_BL::Message>(
            libBLEEP_BL::PeerId(_node->GetIP()), libBLEEP_BL::PeerId(""), "notifyPeerId");
    ...
    _node->SendMsg(conn_fd, BLEEP_MAGIC);
    _node->SendMsg(conn_fd, serial_str.size());
    _node->SendMsg(conn_fd, serial_str);
...
void BLNodePrimitives::OpAfterRecv(int data_fd, std::string recv_str) {
    ...
    // recv to RecvBuffer
    TCPControl &tcpControl = _node->GetTCPControl(data_fd);
            ...
            if (msg_size && recvbufstr.size() >= BLEEP_MAGIC_SIZE + sizeof(int) + msg_size) {
                ...
                if (msg->GetType() == "PING") {
                    ...
                    std::shared_ptr<libBLEEP_BL::Message> pongMsg = std::make_shared<libBLEEP_BL::Message>(
                            libBLEEP_BL::PeerId(_node->GetIP()), libBLEEP_BL::PeerId(""), "PONG");
                    ...
                    _node->SendMsg(data_fd, BLEEP_MAGIC);
                    _node->SendMsg(data_fd, serial_str.size());
                    _node->SendMsg(data_fd, serial_str);
                }

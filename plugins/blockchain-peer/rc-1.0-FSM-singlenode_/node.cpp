...
void *writer_ABC(void*)
{
    ...
    while(1){
        result = write (idleState.GetPipeWriteFD(), &ch,1);
        if (result != 1){
            perror ("write");
            exit (2);
        }
    ...
    // sleep(1);
    close(idleState.GetPipeWriteFD());
...
int main(int argc, char *argv[]) {
    ...
    StateMachine* blockchainStateMachine = new StateMachine();


    /* init state machine.  */
    blockchainStateMachine->InitStateMachine();
    ...
    blockchainStateMachine->StartStateMachine();

    delete(blockchainStateMachine);

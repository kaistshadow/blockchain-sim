...
void HandleNetwork_GossipModel::onRecvSocketDataEvent(std::shared_ptr<EventInfo> info) {
    ...
    if (is_disconnect) {
        ...
        Membership_HandleDisconnect(peer);
...
int HandleNetwork_GossipModel::JoinNetwork() {
    ...
    int result = Connect("bleep0");
    if (amIFullNode) {
        Peer* contactnode = GetPeerFromPeerMap("bleep0");
...
int HandleNetwork_GossipModel::Connect(std::string targetid) {
    if (GetPeerFromPeerMap(targetid)) {
        std::cout << "[Connect] target already exists\n";
        return 0;
    }
...
void HandleNetwork_GossipModel::Membership_HandleDisconnect(Peer* peer) {
    std::string src = peer->GetIP();
    if (PartialViewManager.FindPeerFromActive(src)) {
        std::cout << "[Membership_HandleDisconnect] activeview timeout with "<<src<<"\n";
   ...
   // treat PRUNE, IHAVE, GRAFT
    ...
    if (ptype == PROTOCOL_CONSENSUS) {
        ...
        if (!neighbor) {
            if (header->IsInjected()) {
                std::cout << "NEWMSG " << utility::GetCurrentTime() << ' ' << mid << '\n';
    ...
    switch(msg->GetType()) {
        case Message::TEST_MESSAGE:
         {
            //std::cout << "[Gossip_RunProtocol] MESSAGE_TYPE TEST_MESSAGE" << "\n";
            delete header;
        }
        break;
        ...
        {
            //std::cout << "[Gossip_RunProtocol] MESSAGE_TYPE TX_MESSAGE" << "\n";
        ...
        {
            //std::cout << "[Gossip_RunProtocol] MESSAGE_TYPE CONSENSUS_MESSAGE" << "\n";
            ConsensusMessage *conmsg = dynamic_cast<ConsensusMessage*>(msg);
            handleConsensusClass->HandleArrivedConsensusMsg(conmsg);
            delete header;
        }
...
void TimeoutList::ConnectionTimeoutCallback(EV_P_ ev_timer *w, int revents) {
    ...
    instance->Membership_HandleDisconnect(peer);
...
void PartialView::DropFromActive(std::string id) {
...
void PartialView::DropFromPassive(std::string id) {

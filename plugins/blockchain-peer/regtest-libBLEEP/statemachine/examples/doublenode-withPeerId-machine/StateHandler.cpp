...
void doublenode_blockchain_machine::RegisterStateHandlers() {
    ...
    gStateMachine.stateToSignalMap[StateEnum::broadcastConsensusMsg].connect(&broadcastConsensusMsgStateHandler);
...
StateEnum doublenode_blockchain_machine::shadowPipeEventNotifiedStateHandler() {
    ...
    if (cmd_id == "generateTx") {
        ...
        for (std::shared_ptr<DataSocket> sock : gStateMachine.dataSocketManager.GetAllDataSockets()) {
            std::shared_ptr<Message> msg = std::make_shared<Message>(*gStateMachine.peerManager.GetMyPeerId(), 
                                                                     PeerId("all"),
                                                                     "StringMessage", cmd_args_str);
            sock->AppendMessageToSendBuff(msg);
        }
    } else if (cmd_id == "setMyId") {
        gStateMachine.peerManager.InitMyPeerId(cmd_args_str);
    }
...
StateEnum doublenode_blockchain_machine::receiveConnectionStateHandler() {
    StateEnum nextState = StateEnum::idle;
    PeerManager& peerManager = gStateMachine.peerManager;
    ListenSocketManager& listenSocketManager = gStateMachine.listenSocketManager;
    DataSocketManager& dataSocketManager = gStateMachine.dataSocketManager;
    ...
    while (1) {
        ...
        // Send id exchange message to newly create dataSocket
        std::shared_ptr<Message> msg = std::make_shared<Message>(*peerManager.GetMyPeerId(), 
                                                                 PeerId(),
                                                                 "IdExchange");
        gStateMachine.dataSocketManager.GetDataSocket(data_sfd)->AppendMessageToSendBuff(msg);
    }
...
StateEnum doublenode_blockchain_machine::socketConnectedStateHandler() {
    ...
    std::shared_ptr<Message> msg = std::make_shared<Message>(*peerManager.GetMyPeerId(), 
                                                             PeerId(),
                                                             "IdExchange");
    gStateMachine.dataSocketManager.GetDataSocket(fd)->AppendMessageToSendBuff(msg);
...
StateEnum doublenode_blockchain_machine::readableSocketStateHandler() {
    ...
    if (message) {
        ...
        else if (messageType == "IdExchange") {
            std::cout << "receive IdExchange" << "\n";
            PeerId neighborId = message->GetSource();
            std::shared_ptr<PeerInfo> peerInfo = gStateMachine.peerManager.AppendNewNeighborPeer(neighborId);
            peerInfo->SetSocketFD(fd);
            peerInfo->SetSocketStatus(SocketStatus::SocketConnected);
        }
    }
        ...
        case SocketEventEnum::closeEvent:
            {
                ...
                std::shared_ptr<PeerInfo> peerInfo = peerManager.GetPeerInfoBySocket(socketFD);
                peerInfo->SetSocketStatus(SocketStatus::SocketDisconnected);
                peerInfo->SetSocketFD(socketFD);
                break;
            }
        }
    }
...
StateEnum doublenode_blockchain_machine::appendBlockStateHandler() {
    ...
    nextState = StateEnum::broadcastConsensusMsg;

    return nextState;
}

StateEnum doublenode_blockchain_machine::broadcastConsensusMsgStateHandler() {
    StateEnum nextState = StateEnum::idle;

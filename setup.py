...
if __name__ == '__main__':
    ...
    if OPT_EOS:
        ...
        prepare_shadow()
        prepare_shadow_dependencies()
        cmake_eos_opt = "-DEOS_OPT=ON"
        cmake_bleeplib_opt = "-DBLEEPLIB_OPT=OFF"
        cmake_bitcoin_opt = "-DBITCOIN_OPT=OFF"
        exec_shell_cmd("mkdir build; cd build; cmake %s %s %s %s ../; cmake --build . --target install -- -j 8; cd ..;" %(cmake_debug_opt, cmake_eos_opt, cmake_bleeplib_opt,cmake_bitcoin_opt))
        process_ENV()

...
def setup_multiple_node_xml(node_num, injector_op):
    ...
    for i in range(0, node_num):
        ...
        if i == 0:
            ...
            #shutdownTime = str(100-int(time))
            shutdownTime = str(150)
            argument = "-handlenetwork=gossip -gossipshutdown -shutdownTime="+shutdownTime
        ET.SubElement(node,"application", plugin="PEER", time=time, arguments=argument)

    if injector_op:
        node_id = "bleep%d" % node_num
        node = ET.SubElement(shadow, "node", id=node_id)
        ET.SubElement(node, "application", plugin="PEER", time=str(node_num+160), arguments="-handlenetwork=gossip -networkparticipant -generatetx=4 -timegeneratetx=1")
...
if __name__ == '__main__':
    ...
    setup_multiple_node_xml(nodenum, True)
    datadir = "rc1-datadir"

...
def setup_multiple_node_xml(node_num, injector_op):
    ...
    for node in shadow.findall('kill'):
        shadow.remove(node)
    ET.SubElement(shadow, "kill", time=str(node_num+350))
    ...
    for i in range(0, node_num):
        ...
        if i == 0:
            ...
            shutdownTime = str(140+i)
            argument = "-handlenetwork=gossip -shutdownTime="+shutdownTime
        ET.SubElement(node,"application", plugin="PEER", time=time, arguments=argument)

    node_id = "bleep%d" % node_num
    node = ET.SubElement(shadow, "node", id=node_id)
    ET.SubElement(node, "application", plugin="PEER", time=str(130), arguments="-handlenetwork=gossip -networkparticipant -generatetx=1 -timegeneratetx=1")
    node_id = "bleep%d" % (node_num+1)
    node = ET.SubElement(shadow, "node", id=node_id)
    ET.SubElement(node, "application", plugin="PEER", time=str(300), arguments="-handlenetwork=gossip -networkparticipant -generatetx=1 -timegeneratetx=1")

...
def setup_multiple_node_xml(node_num, client_num, simultime, txgeninterval, txnum, miningtime):
    base_xml = "base.xml"
    ...
    for i in range(0, node_num):
        ...
        time = str(0)
        argument = "-txgeninterval=1000000000 -outpeernum=20 -fanout=7 -blocktxnum=%d -id=bleep%d -l=debug -miningtime=%d -miningnodecnt=%d " % (txnum, i, miningtime, node_num)
        if(i < ((node_num-1)//6)):
